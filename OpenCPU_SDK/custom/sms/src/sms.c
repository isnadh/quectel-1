...
************************************************************************/
static bool IsValidConParam(ST_SMS_Con *pCon);
...
static bool ConvDeliverSMSParamToTextInfo(LIB_SMS_DeliverPDUParamStruct *pSMSParam,ST_SMS_DeliverParam *pTextInfo);
static bool ConvSubmitSMSParamToTextInfo(LIB_SMS_SubmitPDUParamStruct *pSMSParam,ST_SMS_SubmitParam *pTextInfo);
static bool ConvSMSParamToTextInfo(u8 uCharSet,LIB_SMS_PDUParamStruct *pSMSParam,ST_SMS_TextInfo* pTextInfo);
...
static char* HdlrReadPDUMsg(char *pLine,u32 uLen,ST_SMS_PDUInfo *pPDU);
static char* HdlrSendPDUMsg(char *pLine,u32 uLen,ST_SMS_SendPDUInfo *pInfo);
...
static s32 CmdReadPDUMsg(u32 uIndex, ST_SMS_PDUInfo* pPDU);
...
(   \
    (   \
            (SMS_STORAGE_TYPE_SM == (StorageType))  \
    ) ? TRUE : FALSE   \
)
...
(   \
    (   \
            ((((ST_SMS_PDUInfo*)(PDUInfo))->status) <= SMS_STATUS_TYPE_STO_SENT)   \
        &&  ((((ST_SMS_PDUInfo*)(PDUInfo))->length) <= sizeof(((ST_SMS_PDUInfo*)(PDUInfo))->data)) \
    ) ? TRUE : FALSE   \
)
...
#define SMS_SET_INVALID_PDU_INFO(PDUInfo)   \
    do  \
    {   \
        Ql_memset(((ST_SMS_PDUInfo*)(PDUInfo)),0x00,sizeof(ST_SMS_PDUInfo));    \
        \
        (((ST_SMS_PDUInfo*)(PDUInfo))->status) = SMS_STATUS_TYPE_INVALID;   \
    } while(0)
  
#define SMS_SET_INVALID_TEXT_INFO(TextInfo) \
    do  \
    {   \
        Ql_memset(((ST_SMS_TextInfo*)(TextInfo)),0x00,sizeof(ST_SMS_TextInfo)); \
        \
        (((ST_SMS_TextInfo*)(TextInfo))->status) = SMS_STATUS_TYPE_INVALID; \
    } while(0)

#define SMS_SET_PDU_MODE(ErrCode,DbgFunName)   \
    do  \
    {   \
        (ErrCode) = Ql_SendATCmd("AT+CMGF=0",Ql_strlen("AT+CMGF=0"),NULL,NULL,0);   \
        if(RIL_ATRSP_SUCCESS != (ErrCode))  \
        {   \
            APP_DEBUG("FAIL! AT+CMGF=0 execute error! Code:%d\r\n",(ErrCode)); \
            return (ErrCode);    \
        }   \
    } while(0)
...
#define SMS_HDLR_CHECK_ERROR(pLine,uLen,DbgFunName) \
    ...
    {   \
        ...
        {   \
           APP_DEBUG(",SUCCESS. Find string: \"OK\"\r\n"); \
            return RIL_ATRSP_SUCCESS;   \
        }   \
            \
        if(NULL != Ql_RIL_FindLine((pLine), (uLen), "ERROR"))   \
        {   \
           APP_DEBUG(",SUCCESS. Find string: \"ERROR\"\r\n"); \
            return RIL_ATRSP_FAILED;    \
        }   \
            \
        if(NULL != Ql_RIL_FindString((pLine), (uLen), "+CME ERROR:")) \
        {   \
           APP_DEBUG(",SUCCESS. Find string: \"+CME ERROR:\"\r\n"); \
            return RIL_ATRSP_FAILED;    \
        }   \
            \
        if(NULL != Ql_RIL_FindString((pLine), (uLen), "+CMS ERROR:"))   \
        {   \
           APP_DEBUG(",SUCCESS. Find string: \"+CMS ERROR:\"\r\n"); \
            return RIL_ATRSP_FAILED;    \
        }   \
    } while(0);
...
#define SMS_GET_STORAGE_NAME(StorageType,StorageName)  \
    do  \
    {   \
        if(SMS_STORAGE_TYPE_MT == (StorageType))    \
        {   \
            Ql_strcpy((StorageName),"MT");  \
        }   \
        else if(SMS_STORAGE_TYPE_ME == (StorageType))    \
        {   \
...
* Parameters:    
*               <uStoType>:
*                   [In] SMS storage type,same as: 'Enum_SMS_SMSStorage'
*               <uRILIdx>:
...
static u32 ADP_SMS_ConvIdxToCoreIdx(u8 uStoType,u32 uRILIdx, u32 uRILMaxIdx)
{
    ...
    {
       APP_DEBUG("Enter ADP_SMS_ConvIdxToCoreIdx,FAIL! ADP_IS_SUPPORT_STORAGE_TYPE FAIL! uStoType:%u",uStoType);
        return 0;
    }
    
    if((uRILIdx < 1) || (uRILIdx > uRILMaxIdx))
    {
       APP_DEBUG("Enter ADP_SMS_ConvIdxToCoreIdx,FAIL! uRILIdx:%u,uRILMaxIdx:%u\r\n",uRILIdx,uRILMaxIdx);
        return 0;
    }
    
    uCoreIdx = uRILIdx;
    
   APP_DEBUG("Enter ADP_SMS_ConvIdxToCoreIdx,SUCCESS. uRILIdx:%u,uRILMaxIdx:%u,uCoreIdx:%u\r\n",uRILIdx,uRILMaxIdx,uCoreIdx);
...
static bool IsValidConParam(ST_SMS_Con *pCon)
{
    if(NULL == pCon)
    {
       APP_DEBUG("Enter IsValidConParam,FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    {
       APP_DEBUG("Enter IsValidConParam,FAIL! msgType:%d is INVALID.",pCon->msgType);
        return FALSE;
    }
    ...
    {
       APP_DEBUG("Enter IsValidConParam,FAIL! msgSeg:%d or msgTot: %d INVALID.",pCon->msgSeg,pCon->msgTot);
        return FALSE;
    }
...
static bool ConvStringToPhoneNumber(char *pString,u32 uStrLen,LIB_SMS_PhoneNumberStruct *pNumber)
{
    ...
    {
       APP_DEBUG("Enter ConvStringToPhoneNumber,FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    if(0 == uStrLen)  //Not given number string
    {
       ...
       APP_DEBUG("Enter ConvStringToPhoneNumber,SUCCESS. NOTE: uStrLen is 0.");
        return TRUE;
    }
    ...
    {
       APP_DEBUG("Enter ConvStringToPhoneNumber,FAIL! Phone number is too long. LIB_SMS_PHONE_NUMBER_MAX_LEN:%d,Now:%d",LIB_SMS_PHONE_NUMBER_MAX_LEN,(uStrLen - (pTmp - pString)));
        return FALSE;
    }
    ...
    for(i = 0; i < (uStrLen - (pTmp - pString)); i++)
    {
        ...
        {
           APP_DEBUG("Enter ConvStringToPhoneNumber,FAIL! LIB_SMS_IS_VALID_ASCII_NUMBER_CHAR FAIL! Char[%d]:0x%x",i,pTmp[i]);
            return FALSE;
        }
    }
   ...
   APP_DEBUG("Enter ConvStringToPhoneNumber,SUCCESS. uLen:%d,uType:%d",(pNumber->uLen),uType);
...
static bool ConvPhoneNumberToString(LIB_SMS_PhoneNumberStruct *pNumber,char *pString,u32 uStrLen)
{
    ...
    {
       APP_DEBUG("Enter ConvPhoneNumberToString,FAIL! Parameter is NULL.");
        return FALSE;
    }

    if(0 == (pNumber->uLen))
    {
       APP_DEBUG("Enter ConvPhoneNumberToString,SUCCESS. NOTE: Number length is 0.");
    ...
    {
       APP_DEBUG("Enter ConvPhoneNumberToString,FAIL! Number type is INVALID. uType:%u",(pNumber->uType));
        return FALSE;
    }
    
    if(uStrLen < uLimitLen)
    {
       APP_DEBUG("Enter ConvPhoneNumberToString,FAIL! uStrLen is less than uLimitLen. uStrLen:%u,uLimitLen:%u",uStrLen,uLimitLen);
        return FALSE;
    }
   ...
   APP_DEBUG("Enter ConvPhoneNumberToString,SUCCESS. pString: %s",pString);
...
static bool ConvTimeStampToString(LIB_SMS_TimeStampStruct *pTimeStamp,char *pString,u32 uStrLen)
{
    ...
    {
       APP_DEBUG("Enter ConvTimeStampToString,FAIL! Parameter is NULL.");
        return FALSE;
    }
    
    if(uStrLen < SMS_TIME_STAMP_STR_MAX_LEN)
    {
       APP_DEBUG("Enter ConvTimeStampToString,FAIL! uStrLen is too less. uStrLen:%u,SMS_TIME_STAMP_STR_MAX_LEN:%u",uStrLen,SMS_TIME_STAMP_STR_MAX_LEN);
        return FALSE;
    }
   ...
   APP_DEBUG("Enter ConvTimeStampToString,SUCCESS. pString:%s",pString);
...
*  
* Description:
*               Convert 'LIB_SMS_DeliverPDUParamStruct' data to 'ST_SMS_DeliverParam' data.
*
* Parameters:    
*               <pSMSParam>:
...
static bool ConvDeliverSMSParamToTextInfo(LIB_SMS_DeliverPDUParamStruct *pSMSParam,ST_SMS_DeliverParam *pTextInfo)
{
    ...
    {
       APP_DEBUG("Enter ConvDeliverSMSParamToTextInfo,FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter ConvDeliverSMSParamToTextInfo,FAIL! ConvPhoneNumberToString FAIL!");
        return FALSE;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter ConvDeliverSMSParamToTextInfo,FAIL! ConvTimeStampToString FAIL!");
        return FALSE;
    }    
    
   APP_DEBUG("Enter ConvDeliverSMSParamToTextInfo,SUCCESS. alpha:%u,oa:%s,scts:%s",(pTextInfo->alpha),(pTextInfo->oa),(pTextInfo->scts));
...
*  
* Description:
*               Convert 'LIB_SMS_SubmitPDUParamStruct' data to 'ST_SMS_SubmitParam' data.
*
* Parameters:    
*               <pSMSParam>:
*                   [In] The pointer of SMS parameter
...
static bool ConvSubmitSMSParamToTextInfo(LIB_SMS_SubmitPDUParamStruct *pSMSParam,ST_SMS_SubmitParam *pTextInfo)
{
    ...
    if((NULL == pSMSParam) || (NULL == pTextInfo))
    {
       APP_DEBUG("Enter ConvSubmitSMSParamToTextInfo,FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter ConvSubmitSMSParamToTextInfo,FAIL! ConvPhoneNumberToString FAIL!");
        return FALSE;
    }

   APP_DEBUG("Enter ConvSubmitSMSParamToTextInfo,SUCCESS. alpha:%u,da:%s",(pTextInfo->alpha),(pTextInfo->da));
...
*  
* Description:
*               Convert 'LIB_SMS_PDUParamStruct' data to 'ST_SMS_TextInfo' data
*
* Parameters:    
*               <uCharSet>
...
{
    bool bResult = FALSE;
    ST_SMS_DeliverParam *pDeliverTextInfo = NULL;
    ST_SMS_SubmitParam *pSubmitTextInfo = NULL;

    if((NULL == pSMSParam) || (NULL == pTextInfo))
    {
       APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! ConvPhoneNumberToString FAIL!");
        return FALSE;
    }
    ...
    if(LIB_SMS_PDU_TYPE_DELIVER == (pTextInfo->type))
    {
        ...
        if(FALSE == bResult)
        {
           APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! ConvDeliverSMSParamToTextInfo FAIL!");
            return FALSE;
        }
        ...
        if(FALSE == bResult)
        {
           APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! LIB_SMS_ConvAlphaToCharSet FAIL!");
            return FALSE;
        }
    }
    else if(LIB_SMS_PDU_TYPE_SUBMIT == (pTextInfo->type))
    {
        ...
        if(FALSE == bResult)
        {
           APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! ConvSubmitSMSParamToTextInfo FAIL!");
            return FALSE;
        }
    ...
    {
       APP_DEBUG("Enter ConvSMSParamToTextInfo,FAIL! Msg type is INVALID. type:%d",(pTextInfo->type));
        return FALSE;
    }

   APP_DEBUG("Enter ConvSMSParamToTextInfo,SUCCESS. bResult:%d",bResult);
...
*               <pType>:
*                   [In] The type of SMS storage,same as 'Enum_SMS_SMSStorage'
*
* Return:  
*               TRUE:  This function works SUCCESS.
*               FALSE: This function works FAIL!
...
static bool GetStorageType(char *pStr,u8 *pType)
{
    ...
    {
       APP_DEBUG("Enter GetStorageType FAIL! Parameter is NULL.");
        return FALSE;
    }
    ...
    {
        (*pType) = SMS_STORAGE_TYPE_SM;
    }
    else if(0 == Ql_strcmp(aBuf,"\"ME\""))
    {
        (*pType) = SMS_STORAGE_TYPE_ME;
    }
    else if(0 == Ql_strcmp(aBuf,"\"MT\""))
    {
        (*pType) = SMS_STORAGE_TYPE_MT;
    }
    else
    {
       APP_DEBUG("Enter GetStorageType FAIL! Storage type is INVALID. aBuf:%s",aBuf);
        return FALSE;
    }
    
   APP_DEBUG("Enter GetStorageType SUCCESS. aBuf:%s,*pType:%d",aBuf,(*pType));
...
static char* HdlrGetStorageInfo(char *pLine,u32 uLen,ST_SMSStorage *pInfo)
{
    ...
    {
       APP_DEBUG("Enter HdlrGetStorageInfo FAIL! Parameter is NULL.");
        return NULL;
    }
    ...
    if(NULL == pHead)
    {
       APP_DEBUG("Enter HdlrGetStorageInfo FAIL! NOT find \"" CPMS_KEY_STR "\"");
        return NULL;
    }
    ...
    {
       APP_DEBUG("Enter HdlrGetStorageInfo FAIL! GetStorageType FAIL! string:%c%c%c%c",pHead[0],pHead[1],pHead[2],pHead[3]);
        return NULL;
    }
    ...
    if(NULL == pHead)
    {
       APP_DEBUG("Enter HdlrGetStorageInfo FAIL! Ql_strstr FAIL! NOT find comma.");
        return NULL;
    }
   ...
   APP_DEBUG("Enter HdlrGetStorageInfo SUCCESS. storage:%u,used:%u,total:%u",(pInfo->storage),(pInfo->used),(pInfo->total));
...
static char* HdlrSetStorage(char *pLine,u32 uLen,ST_SMSStorage *pInfo)
{
    ...
    if((NULL == pLine) || (0 == uLen) || (NULL == pInfo))
    {
       APP_DEBUG("Enter HdlrSetStorage FAIL! Parameter is NULL.");
        return NULL;
    }
    ...
    if(NULL == pHead)
    {
       APP_DEBUG("Enter HdlrSetStorage FAIL! NOT find \"" CPMS_KEY_STR "\"");
        return NULL;
    }
    ...
    if(NULL == pTail)
    {
       APP_DEBUG("Enter HdlrSetStorage FAIL! Ql_strstr FAIL! NOT find comma.");
        return NULL;
    }
   ...
   APP_DEBUG("Enter HdlrSetStorage SUCCESS. used:%u,total:%u",(pInfo->used),(pInfo->total));
...
*                   [In] The length of a string
*               <pPDU>:
*                   [In] The pointer of 'ST_SMS_PDUInfo' data
*
* Return:  
*               NULL:  This function works FAIL!
*               OTHER VALUE: This function works SUCCESS
...
static char* HdlrReadPDUMsg(char *pLine,u32 uLen,ST_SMS_PDUInfo *pPDU)
{
    ...
    {
       APP_DEBUG("Enter HdlrReadPDUMsg,FAIL! Parameter is NULL.");
        return NULL;
    }
    ...
    if(TRUE == s_bSMSReadContentFlag)
    {
       ...
       APP_DEBUG("Enter HdlrReadPDUMsg,SUCCESS. status:%u,length:%u,s_bSMSReadContentFlag:%d",(pPDU->status),(pPDU->length),s_bSMSReadContentFlag);
    ...
    if(NULL == pHead)
    {
       APP_DEBUG("Enter HdlrReadPDUMsg FAIL! NOT find \"" CMGR_KEY_STR "\"");
        return NULL;
    }
    ...
    if(NULL == pTail)
    {
       APP_DEBUG("Enter HdlrReadPDUMsg FAIL! Ql_strstr FAIL! NOT find comma.");
        return NULL;
    }
    
    CONV_STRING_TO_INTEGER(pHead,(pTail-pHead),(pPDU->status));
    
   APP_DEBUG("Enter HdlrReadPDUMsg SUCCESS. status:%u,s_bSMSReadContentFlag:%d",(pPDU->status),s_bSMSReadContentFlag);
...
*                   [In] The length of a string
*               <pPDU>:
*                   [In] The pointer of 'ST_SMS_SendPDUInfo' data
*
* Return:  
*               NULL:  This function works FAIL!
*               OTHER VALUE: This function works SUCCESS
...
static char* HdlrSendPDUMsg(char *pLine,u32 uLen,ST_SMS_SendPDUInfo *pInfo)
{
    ...
    if((NULL == pLine) || (0 == uLen) || (NULL == pInfo))
    {
       APP_DEBUG("Enter HdlrSendPDUMsg FAIL! Parameter is NULL.");
        return NULL;
    }
    ...
    if(NULL == pHead)
    {
       APP_DEBUG("Enter HdlrSendPDUMsg FAIL! NOT find \"" CMGS_KEY_STR "\"");
        return NULL;
    }
   ...
   APP_DEBUG("Enter HdlrSendPDUMsg SUCCESS. mr:%u",(pInfo->mr));
...
static s32 SMS_CMD_GeneralHandler(char* pLine, u32 uLen, void* pUserData)
{
    ...
    {
       APP_DEBUG("Enter SMS_CMD_GeneralHandler,FAIL! Parameter is NULL.");
    ...
    switch(uType)
    {
        case HDLR_TYPE_CPMS_READ_CMD:
        {
            ...
            {
               APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CPMS_READ_CMD,storage:%u,used:%u,total:%u",(pInfo->storage),(pInfo->used),(pInfo->total));
        ...
        case HDLR_TYPE_CPMS_SET_CMD:
        {
            ...
            {
               APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CPMS_SET_CMD,used:%u,total:%u",(pInfo->used),(pInfo->total));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        {
            ST_SMS_PDUInfo *pInfo = (ST_SMS_PDUInfo*)(pParam->pUserData);

            if(NULL != HdlrReadPDUMsg(pLine, uLen, pInfo))
            {
               APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CMGR_PDU_CMD,status:%u,length:%u",(pInfo->status),(pInfo->length));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        {
            ST_SMS_SendPDUInfo *pInfo = (ST_SMS_SendPDUInfo*)(pParam->pUserData);

            if(NULL != HdlrSendPDUMsg(pLine, uLen, pInfo))
            {
               APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CMGS_PDU_CMD,length:%u",(pInfo->pduLen));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        default:
        {
           APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. Warning: NOT support type! uType:%u",uType);
        }
        break;
    }
   ...
   APP_DEBUG("Enter SMS_CMD_GeneralHandler,SUCCESS. uType:%u",uType);
...
* Parameters:    
*               <pCurrMem>:
*                   [In] The pointer of current SMS storage type,same as: 'Enum_SMS_SMSStorage'
*               <pUsed>:
...
static s32 CmdGetStorageInfo(u8* pCurrMem, u32* pUsed,u32* pTotal)
{
   ...
   APP_DEBUG("Enter CmdGetStorageInfo,SUCCESS. storage:%d,used:%d,total:%d",(sInfo.storage),(sInfo.used),(sInfo.total));
...
static s32 CmdSetStorageInfo(u8 uStoType,u32* pUsed,u32* pTotal)
{
    ...
    if(FALSE == ADP_IS_SUPPORT_STORAGE_TYPE(uStoType))
    {
       APP_DEBUG("Enter CmdSetStorageInfo,FAIL! ADP_IS_SUPPORT_STORAGE_TYPE FAIL!");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    iResult = Ql_SendATCmd(aCmd, iLen, SMS_CMD_GeneralHandler,&sUserData, 0);
    if(RIL_ATRSP_SUCCESS != iResult)
    {
       APP_DEBUG("Enter CmdSetStorageInfo,FAIL! AT+CPMS? execute error! iResult:%d\r\n",iResult);
        return iResult;
    }
   ...
   APP_DEBUG("Enter CmdGetStorageInfo,SUCCESS. uStoType:%d,used:%d,total:%d",uStoType,(sInfo.used),(sInfo.total));
...
static s32 CmdReadPDUMsg(u32 uIndex, ST_SMS_PDUInfo* pPDU)
{
    ...
    if(NULL == pPDU)
    {
       APP_DEBUG("Enter CmdReadPDUMsg,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(RIL_ATRSP_SUCCESS != iResult)
    {
       APP_DEBUG("Enter CmdReadPDUMsg,FAIL! iResult:%d",iResult);
        return iResult;
    }

    if((uIndex < 1) || (uIndex > uStoTot))
    {
       APP_DEBUG("Enter CmdReadPDUMsg,FAIL! uIndex is INVALID. uIndex:%d",uIndex);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(0 == uCoreIdx)
    {
       APP_DEBUG("Enter CmdReadPDUMsg,FAIL! ADP_SMS_ConvIdxToCoreIdx FAIL! uStoType:%u,index:%u,uStoTot:%u", uStoType,uIndex,uStoTot);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    {
       APP_DEBUG("Enter CmdReadPDUMsg,FAIL! CMD: %s execute FAIL!",aCmd);
        return iResult;
    }

   APP_DEBUG("Enter CmdReadPDUMsg,SUCCESS. CMD: %s,uStoType:%u,uStoTot:%u,index:%u,length:%u", aCmd,uStoType,uStoTot,uIndex,(pPDU->length));
...
static s32 CmdSendPDUMsg(char* pPDUStr,u32 uPDUStrLen,u32 *pMsgRef)
{
    ...
    {
       APP_DEBUG("Enter CmdSendPDUMsg,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter CmdSendPDUMsg,FAIL! LIB_SMS_IsValidHexStr FAIL!");
        return RIL_AT_INVALID_PARAM;
    }

    if((uPDUStrLen % 2) != 0)
    {
       APP_DEBUG("Enter CmdSendPDUMsg,FAIL! (uPDUStrLen % 2) != 0");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    //Set to PDU mode
    SMS_SET_PDU_MODE(iResult,"SMS_ReadSMS_PDU");
    ...
    if(iResult != RIL_ATRSP_SUCCESS)
    {
       APP_DEBUG("Enter CmdSendPDUMsg,FAIL! CMD: %s execute FAIL!",aCmd);
        return iResult;
    }

    LIB_SMS_SET_POINTER_VAL(pMsgRef,(sInfo.mr));

   APP_DEBUG("Enter CmdSendPDUMsg,SUCCESS. mr:%u",(sInfo.mr));
    
    return RIL_ATRSP_SUCCESS;
}

/******************************************************************************
* Function:     SMS_GetStorage
*  
* Description:
*               Get SMS storage info
*
...
******************************************************************************/
s32 SMS_GetStorage(u8* pCurrMem, u32* pUsed,u32* pTotal)
{
    s32 iResult = 0;

    iResult = CmdGetStorageInfo((u8*)pCurrMem,pUsed,pTotal);

   APP_DEBUG("Enter SMS_GetStorage,SUCCESS. iResult:%d",iResult);

    return iResult;
}

/******************************************************************************
* Function:     SMS_SetStorage
*  
* Description:
*               Set SMS storage
*
* Parameters:    
*               <storage>:
*                   [In] SMS storage,same as 'Enum_SMS_SMSStorage'
*
* Return:  
*               RIL_ATRSP_SUCCESS:  This function works SUCCESS.
*               RIL_ATRSP_CONTINUE: Need to wait later AT response
...
*               1. If you DONOT want to get <pUsed>,pTotal value,please set it to NULL.
******************************************************************************/
s32 SMS_SetStorage(Enum_SMS_StorageType eStorage,u32* pUsed,u32* pTotal)
{
    s32 iResult = 0;

    iResult = CmdSetStorageInfo(eStorage,pUsed,pTotal);

   APP_DEBUG("Enter SMS_SetStorage,SUCCESS. iResult:%d",iResult);
    
    return iResult;
}

/******************************************************************************
* Function:     SMS_ReadSMS_PDU
*  
* Description:
*               Read a PDU SMS
*
...
*                                      and then call Ql_RIL_Initialize to initialize RIL.
******************************************************************************/
s32 SMS_ReadSMS_PDU(u32 uIndex, ST_SMS_PDUInfo* pPDUInfo)
{
    s32 iResult = 0;
    bool bResult = FALSE;
    ...
    if(NULL == pPDUInfo)
    {
       APP_DEBUG("Enter SMS_ReadSMS_PDU,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }

    //Initialize
    Ql_memset(pPDUInfo,0x00,sizeof(ST_SMS_PDUInfo));

    iResult = CmdReadPDUMsg(uIndex,pPDUInfo);
    if(iResult != RIL_ATRSP_SUCCESS)
    {
       APP_DEBUG("Enter SMS_ReadSMS_PDU,FAIL! CmdReadPDUMsg FAIL!");
        return iResult;
    }
    ...
    if(0 == (pPDUInfo->length))
    {
       ...
       APP_DEBUG("Enter SMS_ReadSMS_PDU,SUCCESS. NOTE:(pPDUInfo->length) is 0.");
        return RIL_ATRSP_SUCCESS;
    }
    ...
    {
       APP_DEBUG("Enter SMS_ReadSMS_PDU,FAIL! IS_VALID_PDU_INFO FAIL!");
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter SMS_ReadSMS_PDU,WARNING! LIB_SMS_CHECK_PDU_STR FAIL!");
    }

   APP_DEBUG("Enter SMS_ReadSMS_PDU,SUCCESS. status:%u,length:%u",(pPDUInfo->status),(pPDUInfo->length));

    return RIL_ATRSP_SUCCESS;
}

/******************************************************************************
* Function:     SMS_ReadSMS_Text
*  
* Description:
*               Read a TEXT SMS
*
...
*                                      and then call Ql_RIL_Initialize to initialize RIL.
******************************************************************************/
s32 SMS_ReadSMS_Text(u32 uIndex, LIB_SMS_CharSetEnum eCharset,ST_SMS_TextInfo* pTextInfo)
{
    ST_SMS_PDUInfo *pPDUInfo = NULL;
    ...
    if(NULL == pTextInfo)
    {
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }

    //Initialize
    Ql_memset(pTextInfo,0x00,sizeof(ST_SMS_TextInfo));

    if(FALSE == LIB_SMS_IS_SUPPORT_CHARSET(eCharset))
    {
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! LIB_SMS_IS_SUPPORT_CHARSET FAIL! eCharset:%d",eCharset);
        return RIL_AT_INVALID_PARAM;
    }

    pPDUInfo = (ST_SMS_PDUInfo*)Ql_MEM_Alloc(sizeof(ST_SMS_PDUInfo));
    if(NULL == pPDUInfo)
    {
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(ST_SMS_PDUInfo));
        return RIL_AT_FAILED;
    }
    ...
    if(NULL == pSMSParam)
    {
       ...
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUParamStruct));
        return RIL_AT_FAILED;
    }
    ...
    {    
       ...
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! CmdReadPDUMsg FAIL!");
        return iResult;
    }
    ...
    if(FALSE == IS_VALID_PDU_INFO(pPDUInfo))
    {
       ...
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! IS_VALID_PDU_INFO FAIL!");
        return RIL_AT_FAILED;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! LIB_SMS_DecodePDUStr FAIL! PDU length:%u",(pPDUInfo->length));
    ...
    if(FALSE == bResult)
    {
       ...
       APP_DEBUG("Enter SMS_ReadSMS_Text,FAIL! ConvSMSParamToTextInfo FAIL!");
        return RIL_AT_FAILED;
    }
   ...
   APP_DEBUG("Enter SMS_ReadSMS_Text,SUCCESS. status:%u",(pTextInfo->status));

    return RIL_AT_SUCCESS;
}

/******************************************************************************
* Function:     SMS_SendSMS_PDU
*  
* Description:
*               This function is used to send PDU message
*
...
*                1. If you DONOT want to get <pMsgRef> value,please set it to NULL.
******************************************************************************/
s32 SMS_SendSMS_PDU(char* pPDUStr,u32 uPDUStrLen,u32 *pMsgRef)
{
    bool bResult = FALSE;
    s32 iResult = 0;
    ...
    if((NULL == pPDUStr) || (0 == uPDUStrLen))
    {
       APP_DEBUG("Enter SMS_SendSMS_PDU,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(FALSE == bResult)
    {
       APP_DEBUG("Enter SMS_SendSMS_PDU,FAIL! LIB_SMS_CHECK_SUBMIT_PDU_STR_FOR_SEND FAIL!");
        return RIL_AT_INVALID_PARAM;
    }

    iResult = CmdSendPDUMsg(pPDUStr,uPDUStrLen,pMsgRef);

   APP_DEBUG("Enter SMS_SendSMS_PDU,SUCCESS. uPDUStrLen:%u,iResult:%d",uPDUStrLen,iResult);

    return iResult;
}

/******************************************************************************
* Function:     SMS_SendSMS_Text
*  
* Description:
...
*                1. If you DONOT want to get <pMsgRef> value,please set it to NULL.
******************************************************************************/
s32 SMS_SendSMS_Text(char* pNumber, u8 uNumberLen, LIB_SMS_CharSetEnum eCharset, u8* pMsg, u32 uMsgLen,u32 *pMsgRef)
{
...
//    {
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! Parameter is NULL.");
//        return RIL_AT_INVALID_PARAM;
//    }
//
//    if(FALSE == LIB_SMS_IS_SUPPORT_CHARSET(eCharset))
//    {
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! LIB_SMS_IS_SUPPORT_CHARSET FAIL! eCharset:%u",eCharset);
//        return RIL_AT_INVALID_PARAM;
//    }
//
...
//    if(NULL == pParam)
//    {
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUParamStruct));
//        return RIL_AT_FAILED;
//    }
//
...
//        Ql_MEM_Free(pParam);
//
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUInfoStruct));
//        return RIL_AT_FAILED;
//    }
//
//    //Initialize
...
//        Ql_MEM_Free(pInfo);
//
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! ConvStringToPhoneNumber FAIL!");
//        return RIL_AT_FAILED;
//    }
//
//    //Set <PID>
...
//        Ql_MEM_Free(pInfo);
//
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! LIB_SMS_ConvCharSetToAlpha FAIL!");
//        return RIL_AT_FAILED;
//    }
//
...
//        Ql_MEM_Free(pInfo);
//
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! LIB_SMS_EncodeSubmitPDU FAIL!");
//        return RIL_AT_FAILED;
//    }
//
...
//        Ql_MEM_Free(pInfo);
//
//       APP_DEBUG("Enter SMS_SendSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%d",((pInfo->uLen) * 2));
//        return RIL_AT_FAILED;
//    }
//
...
//    iResult = CmdSendPDUMsg(pPDUStr,uPDUStrLen,pMsgRef);
//
//   APP_DEBUG("Enter SMS_SendSMS_PDU,SUCCESS. iResult:%d",iResult);
//
//    Ql_MEM_Free(pInfo);
//    Ql_MEM_Free(pPDUStr);
//
...
*                   [Out] The pointer of message reference number
*               <pExt>
*                   [In]  The pointer of 'ST_SMS_SendExt' data
...
******************************************************************************/
s32 SMS_SendSMS_Text_Ext(char* pNumber, u8 uNumberLen, LIB_SMS_CharSetEnum eCharset, u8* pMsg, u32 uMsgLen,u32 *pMsgRef,ST_SMS_SendExt *pExt)
{
    LIB_SMS_PDUParamStruct *pParam = NULL;
    LIB_SMS_SubmitPDUParamStruct *pSubmitParam = NULL;
    LIB_SMS_PDUInfoStruct *pInfo = NULL;
    ...
    if((NULL == pNumber) || (0 == uNumberLen) || (NULL == pMsg))
    {
       APP_DEBUG("Enter SMS_SendSMS_Text_Ext,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }

    if(FALSE == LIB_SMS_IS_SUPPORT_CHARSET(eCharset))
    {
       APP_DEBUG("Enter SMS_SendSMS_Text_Ext,FAIL! LIB_SMS_IS_SUPPORT_CHARSET FAIL! eCharset:%u",eCharset);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if((pExt != NULL) && (TRUE == pExt->conPres))
    {
        ...
        {
           APP_DEBUG("Enter SMS_SendSMS_Text_Ext,WARNING! msgType:%d is INVALID,use default!",pExt->con.msgType);
            (pExt->con.msgType) = LIB_SMS_UD_TYPE_CON_DEFAULT;
        }

        if(FALSE == IsValidConParam(&(pExt->con)))
        {
           APP_DEBUG("Enter SMS_SendSMS_Text_Ext,FAIL! IsValidConParam FAIL!");
            return RIL_AT_INVALID_PARAM;
        }
    }
    ...
    if(NULL == pParam)
    {
       APP_DEBUG("Enter SMS_SendSMS_Text_Ext,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUParamStruct));
        return RIL_AT_FAILED;
    }
    ...
    if(NULL == pInfo)
    {
       ...
       APP_DEBUG("Enter SMS_SendSMS_Text_Ext,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUInfoStruct));
        return RIL_AT_FAILED;
    }
...
*               flag:
*                   [In] Delete flag , which is one value of 'Enum_SMS_DeleteFlag'.
*
* Return:  
*                RIL_AT_SUCCESS,send AT successfully.
*                RIL_AT_FAILED, send AT failed.
...
*                                      and then call Ql_RIL_Initialize to initialize RIL.
******************************************************************************/
s32 SMS_DeleteSMS(u32 uIndex,Enum_SMS_DeleteFlag eDelFlag)
{
    s32 iResult = 0;
    u8 uStoType = 0;  //Current memory type,same as: 'Enum_SMS_SMSStorage'
    ...
    if(eDelFlag > SMS_DEL_ALL_MSG)
    {
       APP_DEBUG("Enter SMS_DeleteSMS,FAIL! eDelFlag is INVALID. eDelFlag:%d", eDelFlag);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(iResult != RIL_ATRSP_SUCCESS)
    {
       APP_DEBUG ("Enter SMS_DeleteSMS,FAIL! CmdGetStorageInfo FAIL! AT+CPMS? execute FAL!");
        return RIL_AT_INVALID_PARAM;
    }

    //Check parameter <uIndex>
    if((SMS_DEL_INDEXED_MSG == eDelFlag)
        && ((uIndex < 1) || (uIndex > uStoTot))
    )
    {
       APP_DEBUG("Enter SMS_DeleteSMS,FAIL! uIndex is INVALID. eDelFlag:%d,uIndex:%u,uCurrStoTot:%u", eDelFlag,uIndex,uStoTot);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(SMS_DEL_INDEXED_MSG == eDelFlag)
    {
        ...
        if(0 == uCoreIdx)
        {
           APP_DEBUG("Enter SMS_DeleteSMS,FAIL! ADP_SMS_ConvIdxToCoreIdx FAIL! uCurrStoType:%u,uIndex:%u,uCurrStoTot:%u", uStoType,uIndex,uStoTot);
            return RIL_AT_INVALID_PARAM;
        }
    }
    else
    ...
    //Set to PDU mode
    SMS_SET_PDU_MODE(iResult,"SMS_DeleteSMS");
    
    //Delete SMS use AT+CMGD=<index>,<delflag>    
    iLen = Ql_sprintf(aCmd, "AT+CMGD=%d,%d", uIndex,eDelFlag);
    iResult = Ql_SendATCmd(aCmd, iLen, NULL, NULL, 0);
    
   APP_DEBUG("Enter SMS_DeleteSMS,SUCCESS. CMD: %s,uStoType:%u,uStoTot:%u,eDelFlag:%u,uIndex:%u", aCmd,uStoType,uStoTot,eDelFlag,uIndex);
